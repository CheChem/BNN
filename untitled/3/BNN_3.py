...
def main(_):
  ...
  with tf.variable_scope("posterior"):
    with tf.variable_scope("qW_0"):
      loc0 = tf.get_variable("loc", [2, 2], initializer=tf.constant_initializer(-300))
      scale0 = tf.nn.softplus(tf.get_variable("scale", [2, 2], initializer=tf.constant_initializer(math.log(math.e**10-1))))
      qW_0 = Normal(loc=loc0, scale=scale0)
    with tf.variable_scope("qW_1"):
      loc1 = tf.get_variable("loc", [2, 2], initializer=tf.constant_initializer(18))
      scale1 = tf.nn.softplus(tf.get_variable("scale", [2, 2], initializer=tf.constant_initializer(math.log(math.e**10-1))))
      qW_1 = Normal(loc=loc1, scale=scale1)
    with tf.variable_scope("qW_2"):
      loc2 = tf.get_variable("loc", [2, 1], initializer=tf.constant_initializer(0.5))
      scale2 = tf.nn.softplus(tf.get_variable("scale", [2, 1], initializer=tf.constant_initializer(math.log(math.e**10-1))))
