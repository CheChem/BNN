...
def main(_):
  ...
  def neural_network(X):
    h = nonlin(tf.matmul(X, W_0) )
    h = nonlin(tf.matmul(h, W_1) )
    h = tf.matmul(h, W_2)
  ...
  with tf.variable_scope("posterior"):
    with tf.variable_scope("qW_0"):
      loc0 = tf.get_variable("loc", [2, 2],initializer=tf.constant_initializer(12))
    ...
    with tf.variable_scope("qW_1"):
      loc1 = tf.get_variable("loc", [2, 2],initializer=tf.constant_initializer(18))
    ...
    with tf.variable_scope("qW_2"):
      loc2 = tf.get_variable("loc", [2, 1],initializer=tf.constant_initializer(13))
