...
def main(_):
  ...
  with tf.name_scope("model"):
    W_0 = Normal(loc=3*tf.ones([2, 2]), scale=tf.ones([2, 2]),
                 name="W_0")
    W_1 = Normal(loc=-4*tf.ones([2, 2]), scale=tf.ones([2, 2]), name="W_1")
    W_2 = Normal(loc=2*tf.ones([2, 1]), scale=tf.ones([2, 1]), name="W_2")
  ...
  with tf.variable_scope("posterior"):
    with tf.variable_scope("qW_0"):
      loc0 = tf.get_variable("loc", [2, 2], initializer=tf.constant_initializer(3))
      scale0 = tf.nn.softplus(tf.get_variable("scale", [2, 2], initializer=tf.constant_initializer(math.log(math.e-1))))
      qW_0 = Normal(loc=loc0, scale=scale0)
    with tf.variable_scope("qW_1"):
      loc1 = tf.get_variable("loc", [2, 2], initializer=tf.constant_initializer(-4))
      scale1 = tf.nn.softplus(tf.get_variable("scale", [2, 2], initializer=tf.constant_initializer(math.log(math.e-1))))
      qW_1 = Normal(loc=loc1, scale=scale1)
    with tf.variable_scope("qW_2"):
      loc2 = tf.get_variable("loc", [2, 1], initializer=tf.constant_initializer(2))
      scale2 = tf.nn.softplus(tf.get_variable("scale", [2, 1], initializer=tf.constant_initializer(math.log(math.e-1))))
