...
W_=np.loadtxt('W_4.txt',delimiter=",")
W=W_.tolist()
for i in range(100):
    model = Sequential()
    model.add(Dense(input_dim=2, units=3, use_bias=False, activation='tanh', kernel_initializer=initializers.random_normal(mean=0, stddev=1)))
    model.add(Dense(input_dim=3, units=2, use_bias=False, activation='tanh', kernel_initializer=initializers.random_normal(mean=0, stddev=1)))
...
WW = np.vstack(W)
np.savetxt('W_4.txt', WW, delimiter=",")
