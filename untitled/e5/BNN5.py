...
def main(_):
  ...
  def neural_network(X):
    h = tf.nn.relu(tf.matmul(X, W_0))
    h = tf.nn.relu(tf.matmul(h, W_1))
    h = tf.matmul(h, W_2)
  ...
  with tf.name_scope("model"):
    W_0 = Normal(loc=tf.zeros([2, 3]), scale=tf.ones([2, 3]),
                 name="W_0")
    W_1 = Normal(loc=tf.zeros([3, 2]), scale=tf.ones([3, 2]), name="W_1")
  ...
  with tf.variable_scope("posterior"):
    with tf.variable_scope("qW_0"):
      loc0 = tf.get_variable("loc", [2, 3])
      scale0 = tf.nn.softplus(tf.get_variable("scale", [2, 3]))
      qW_0 = Normal(loc=loc0, scale=scale0)
    with tf.variable_scope("qW_1"):
      loc1 = tf.get_variable("loc", [3, 2])
      scale1 = tf.nn.softplus(tf.get_variable("scale", [3, 2]))
