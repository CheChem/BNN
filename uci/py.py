...
class Net(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        relu_result = torch.tanh(hidden_result)
...
np.savetxt('y_data.txt',y_data,delimiter=",")
np.savetxt('y_data_real.txt',y_data_real,delimiter=",")
...
for i in range(TRAIN_TIMES):
    ...
    if i % 500 == 0:
        ...
        plt.cla()
        # 有误差散点
        plt.plot(x_data.numpy(), y_data_real.numpy(), c='green')
        # 有误差散点
...
x = torch.unsqueeze(torch.linspace(-0.01, 0.01, 1000), dim=1)
y = net(x)
print(y)